 ...
 */
function publicTransitSearchStations(platform, at) {
...
function createUIforDropdown() {
  ...
  subTitle.innerHTML= "<p>This example shows public transit stations within 1 km radius of below location</p>";
  ...
  function eventCities(){
    ...
    publicTransitSearchStations(platform, cities[citiesIndex].position)
  }
...
function onSuccess(result) {
  ...
  */
  addLocationsToMap(stations);
  addLocationsToPanel(stations);
  // ... etc.
}
 ...
 */
function addLocationsToPanel(stations){
  ...
  for (i = 0;  i < stations.length; i += 1) {
    let station = stations[i],
        ...
        content =  '<strong style="font-size: large;">' + station.place.name  + '</strong></br>';

        if (station.transports){
        transport = station.transports[0];

      content += '<strong>transport mode:</strong> ' + transport.mode + '<br/>';
      content += '<strong>transpost name:</strong> '  + transport.name + '<br/>';
      content += '<strong>head sign:</strong> '  + transport.headsign + '<br/>';
        }
 ...
 *                             H.service.GeocodingService
 */
function addLocationsToMap(stations){
  ...
  for (i = 0;  i < stations.length; i += 1) {
    let station = stations[i];
    marker = new H.map.Marker(station.place.location);
    marker.label = station.place.name;
...
publicTransitSearchStations(platform, cities[0].position);
createUIforDropdown();
